...
// studentId) vive en la tabla `profiles`, creada por trigger al registrarse.
async function cloudLogin(email: string, password: string): Promise<{ ok: boolean; error?: string; user?: User }> {
  ...
  return {
    ...
    user: {
      id: uid,
      role: (p.role as User['role']) ?? 'estudiante',
      nombre: p.nombre ?? '',
      apellido: p.apellido ?? '',
      email: email.trim(),
      password: '',
...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      logout: () => {
        if (isCloudEnabled()) {
          import('@/services/cloud/client').then(({ getSupabase }) => getSupabase().then((sb) => sb.auth.signOut()))
        }
        set({ user: null })
      },
      ...
      updateProfile: async (patch) => {
        ...
        if (isCloudEnabled()) {
          const { getSupabase } = await import('@/services/cloud/client')
          const sb = await getSupabase()
          // Función segura del lado del servidor: solo puede tocar estos 4
          // campos de la propia fila, nunca "role" ni "membershipExpiresAt".
          const { error } = await sb.rpc('me_update_own_profile', {
